...
export function useUpdateSystemWord() {
  const api = useApi();
  const queryClient = useQueryClient();
  const { setStatusMessage, setErrorMessage } = useStoreCallbacks();

  return useMutation({
    mutationFn: ({ word, translation }: { word: string; translation: string }) =>
      api.updateSystemWord(word, translation),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["systemWords"] });
      setStatusMessage("תרגום המילה במאגר עודכן בהצלחה.");
    },
    onError: (err: Error) => setErrorMessage(err.message),
  });
}

export function useDeleteSystemWord() {
  const api = useApi();
  const queryClient = useQueryClient();
  const { setStatusMessage, setErrorMessage } = useStoreCallbacks();

  return useMutation({
    mutationFn: (word: string) => api.deleteSystemWord(word),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["systemWords"] });
      setStatusMessage("המילה נמחקה מהמאגר בהצלחה.");
    },
    onError: (err: Error) => setErrorMessage(err.message),
  });
}

export function useAdminUserLibrary(userId: string) {
  const api = useApi();
  return useQuery({
    queryKey: ["adminUserLibrary", userId],
    queryFn: () => api.getUserLibrary(userId, "", 200),
    enabled: !!userId,
  });
}
