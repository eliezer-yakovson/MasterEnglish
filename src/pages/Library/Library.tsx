...
export default function Library() {
  ...
  function openTranslationEdit(item: LibraryItem) {
    setEditingTranslationId(item.word_id);
    setEditingTranslationValue(item.translation);
    setExpandedWordId(null);
  }

  function handleTranslationSave(e: React.FormEvent, wordId: string) {
    e.preventDefault();
    if (!editingTranslationValue.trim()) return;
    updateMutation.mutate({ wordId, body: { translation: editingTranslationValue.trim() } });
    setEditingTranslationId(null);
  }
            ...
            <div className="library-card" key={item.word_id}>
              ...
              <div className="card-actions" style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
                ...
                <button
                  ...
                  title="ערוך תרגום"
                  onClick={() =>
                    editingTranslationId === item.word_id
                      ? setEditingTranslationId(null)
                      : openTranslationEdit(item)
                  }
                >
                  ✏️
                </button>
              ...
              {editingTranslationId === item.word_id && (
                <form
                  ...
                  onSubmit={(e) => handleTranslationSave(e, item.word_id)}
                ...
                >
                  <label style={{ fontSize: "0.8rem" }}>
                    ...
                    <input
                      ...
                      style={{ marginRight: "0.4rem", width: "100%", marginTop: "0.3rem" }}
                      autoFocus
                    />
                  </label>
