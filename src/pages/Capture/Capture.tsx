...
import { useEnrichAndCheck, useCaptureWord, useUpdateWord } from "../../hooks/useQueries";
...
export default function Capture() {
  ...
  function handleCapturePreview(event: React.FormEvent) {
    ...
    enrichMutation.mutate(
      ...
      { onSuccess: (data) => { setCapturePreview(data); setEditableTranslation(data.enriched.translation); } },
    );
  }
    ...
    captureMutation.mutate(
      { word: captureForm.word, context: captureForm.context || undefined },
      {
        onSuccess: (result) => {
          ...
          if (capturePreview && editableTranslation.trim() && editableTranslation !== capturePreview.enriched.translation) {
            updateWordMutation.mutate({ wordId: result.word_id, body: { translation: editableTranslation.trim() } });
          }
        },
      },
    );
          ...
          <div className="content-card preview-card">
            ...
            <p className="preview-translation">
              <input
                ...
                className="translation-edit-input"
                title="ניתן לערוך את התרגום לפני השמירה"
              />
            </p>
